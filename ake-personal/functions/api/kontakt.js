/**
 * Cloudflare Pages Function — pranon formularin dhe dergon email.
 *
 * Rruga:  POST /api/kontakt        (emri i file-it percakton rrugen)
 *
 * KONFIGURIMI (Cloudflare Dashboard -> Pages -> projekti -> Settings ->
 * Environment variables). Vendosini si "Secret", jo si tekst i thjeshte:
 *
 *   BREVO_API_KEY   celesi nga brevo.com  ->  Settings -> SMTP & API -> API keys
 *   MAIL_TO         info@ake-personal.de
 *   MAIL_FROM       noreply@ake-personal.de   (domain i verifikuar te Brevo)
 *
 * PSE BREVO: eshte kompani franceze me serverë ne BE dhe ofron
 * Auftragsverarbeitungsvertrag (Art. 28 DSGVO). Nje ofrues amerikan do te
 * kerkonte klauzola standarde kontraktuale dhe nje seksion me shume te
 * Datenschutzerklärung. Falas: 300 email/dite.
 *
 * NDRYSHIMI I OFRUESIT: vetem funksioni sendMail() me poshte prek API-n.
 */
...
function reply(request, status, message, redirectTo) {
  ...
  // Pa JavaScript: ridrejtim te faqja e faleminderimit ose kthim me gabim
  if (status < 400 && redirectTo) {
...
async function sendMail(env, { vorname, nachname, email, telefon, nachricht }) {
  ...
  const res = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      ...
      // Pergjigju direkt derguesit kur shtypet "Reply" ne inbox
...
export async function onRequestPost({ request, env }) {
  ...
  if (!env.BREVO_API_KEY || !env.MAIL_TO || !env.MAIL_FROM) {
    console.error('Mungojne variablat e mjedisit: BREVO_API_KEY / MAIL_TO / MAIL_FROM');
  ...
  try {
    ...
    return reply(request, 400, 'Ungültige Anfrage.');
  }

  // Honeypot: botet e mbushin, perdoruesit nuk e shohin.
  // Pergjigju me sukses qe boti te mos mesoje asgje.
  ...
  if (errors.length) {
    return reply(request, 422, `Bitte prüfen Sie Ihre Eingaben: ${errors.join(' ')}`);
  }
...
/** GET ne kete rruge nuk ka kuptim. */
