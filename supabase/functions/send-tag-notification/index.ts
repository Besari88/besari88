...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    const { to, memberName, taggedBy, itemType, itemName, note } = await req.json();
    ...
    const html = `
      <div style="font-family:system-ui,Segoe UI,Arial,sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#1a1a1a">
        ...
        <p style="margin:0 0 8px;font-size:14px"><strong>${by}</strong> hat dich bei der ${kind} <strong>${name}</strong> getaggt.</p>
        ${noteHtml}
        ${buttonHtml}
